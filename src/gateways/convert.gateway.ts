...
export class ConvertGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleApproval(
        ...
        try {
            ...
            const postOfSender = await this.postService.findOneById(payload.convert.activeUserId, ['user'])
            ...
            if (postOfSender.user.id === payload.sender.id) {
                ...
                const indexOfActivePostId = payload.convert.pathOfPosts.indexOf(payload.convert.activeUserId);
                if (payload.isApproved) {
                    ...
                    const nextPost = await this.postService.findOneById(payload.convert.pathOfPosts[indexOfActivePostId + 1], ['user']); // МОЖЕТ БЫТЬ NULL когда последний пост, соответственно аккуратно с nextPost.user.id
                    ...
                    const previousPost = await this.postService.findOneById(payload.convert.pathOfPosts[indexOfActivePostId - 1], ['user']);
    ...
    async handleSeenCoordination(
        ...
        try {
            ...
            if (postOfSender.user.id === payload.sender.id) {
                ...
                const indexOfActivePostId = payload.convert.pathOfPosts.indexOf(payload.convert.activeUserId);
                const convertUserIds = payload.convert.convertToUsers.map(convertToUser => convertToUser.user.id);
                const nextPost = await this.postService.findOneById(payload.convert.pathOfPosts[indexOfActivePostId + 1], ['user']);
                convertUserIds.push(nextPost.user.id);
                const convertUpdateDto: ConvertUpdateDto = {
                    _id: payload.convert.id,
                    userIds: convertUserIds,
