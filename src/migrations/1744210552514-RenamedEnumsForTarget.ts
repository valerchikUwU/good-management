...
export class RenamedEnumsForTarget1744210552514 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."target_type_enum" AS ENUM('Задача', 'Метрика', 'Правила', 'Продукт', 'Организационные мероприятия', 'Приказ', 'Личная')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "target" ALTER COLUMN "type" SET DEFAULT 'Задача'`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."target_targetstate_enum" AS ENUM('Черновик', 'Активная', 'Отменена', 'Завершена')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "target" ALTER COLUMN "targetState" SET DEFAULT 'Черновик'`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."target_targetstate_enum_old" AS ENUM('Активная', 'Завершена', 'Отменена')`,
    );
    await queryRunner.query(
      `ALTER TABLE "target" ALTER COLUMN "targetState" DROP DEFAULT`,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."target_type_enum_old" AS ENUM('Личная', 'Обычная', 'Организационные мероприятия', 'Правила', 'Приказ', 'Продукт', 'Статистика')`,
    );
    await queryRunner.query(
      `ALTER TABLE "target" ALTER COLUMN "type" DROP DEFAULT`,
    );
