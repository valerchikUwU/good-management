...
export class FileService {
    ...
    async findAllForPolicy(policy: PolicyReadDto): Promise<FileReadDto[]> {
        try {
            const files = await this.fileRepository.find({ where: { policy: { id: policy.id } } });
            ...
            throw new InternalServerErrorException('Ошибка при получении всех файлов!');
        }
    }
    ...
    async create(fileCreateDto: FileCreateDto): Promise<File> {
        try {
            ...
            if (!fileCreateDto.mimetype) {
                throw new BadRequestException('У файла обязательно наличие расширения!');
            }
            const file = new File();
