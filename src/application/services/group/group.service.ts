...
export class GroupService {
    ...
    async findAllForAccount(account: AccountReadDto): Promise<GroupReadDto[]> {
        try {
            const groups = await this.groupRepository.find({ where: { account: { id: account.id } } });
            ...
            throw new InternalServerErrorException('Ошибка при получении всех групп!');
        }
    }
    ...
    async create(groupCreateDto: GroupCreateDto): Promise<string> {
        try {
            ...
            await this.groupToUserService.createSeveral(createdGroup, groupCreateDto.groupToUsers);
    ...
    async update(_id: string, updateGroupDto: GroupUpdateDto): Promise<string> {
        try {
            ...
            if (updateGroupDto.groupToUsers) {
                ...
                await this.groupToUserService.createSeveral(group, updateGroupDto.groupToUsers);
            }
            await this.groupRepository.update(group.id, {groupName: group.groupName});
            return group.id
        }
        catch (err) {
