...
export class StatisticDataService {
  ...
  ): Promise<StatisticDataReadDto[]> {
    try {
      const statisticDatas = await this.statisticDataRepository.find({
        where: { statistic: { id: statistic.id } },
      });
      ...
      throw new InternalServerErrorException(
        'Ошибка при получении всех данных по статистике!',
      );
    }
  }
