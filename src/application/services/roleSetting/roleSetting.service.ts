...
export class RoleSettingService {
    ...
    async findAll(): Promise<RoleSettingReadDto[]> {
        ...
        try {
            ...
            throw new InternalServerErrorException('Ошибка при получении настроек доступа');
        }
    }

    async findByRoleAndModule(roleId: string, module: Modules): Promise<RoleSettingReadDto> {

        try {
            const roleSetting = await this.roleSettingRepository.findOne({ where: { role: { id: roleId }, module: module },  relations: ['role'] });

            if (!roleSetting) throw new NotFoundException(`Настройка с ID роли: ${roleId} не найдены`);
    ...
    async create(roleSettingCreateDto: RoleSettingCreateDto): Promise<RoleSetting> {
        try {
            // Проверка на наличие обязательных данных
            if (!roleSettingCreateDto.module) {
                throw new BadRequestException('Выберите модуль, права доступа которого вы хотите ограничить!');
            }
    ...
    async update(_id: string, updateRoleSetting: RoleSettingUpdateDto): Promise<RoleSettingReadDto> {
        try {
            ...
            if (!roleSetting) throw new NotFoundException(`Не найдена настройка с таким ID: ${_id}`);

            if (updateRoleSetting.can_create !== undefined) roleSetting.can_create = updateRoleSetting.can_create;
            if (updateRoleSetting.can_read !== undefined) roleSetting.can_read = updateRoleSetting.can_read;
            if (updateRoleSetting.can_update !== undefined) roleSetting.can_update = updateRoleSetting.can_update;
            ...
            // Обработка других ошибок
            throw new InternalServerErrorException('Ошибка при обновлении настройки доступа!');
        }
