    ...
    async createSeveral(group: Group, userIds: string[]): Promise<GroupToUser[]> {
        ...
        for (const userId of userIds) {
            try {
                ...
                const savedRelation = await this.groupToUserRepository.save(groupToUser);
                ...
                throw new InternalServerErrorException('Ой, что - то пошло не так при добавлении юзера к группе!')
