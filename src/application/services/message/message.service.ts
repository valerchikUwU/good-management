...
export class MessageService {
    ...
    async findAllForConvert(convertId: string): Promise<MessageReadDto[]> {
        const messages = await this.messageRepository.find({ where: { convert: { id: convertId } } })
    ...
    async update(_id: string, messageUpdateDto: MessageUpdateDto): Promise<Message> {

        try {
            const message = await this.messageRepository.findOne({ where: { id: _id } });
            if (!message) {
