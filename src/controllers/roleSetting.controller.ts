import { Controller, Get, Post, Delete, Param, Body, HttpStatus, Patch, Ip, Inject } from '@nestjs/common';
...
export class RoleSettingController {
  constructor(private readonly roleSettingService: RoleSettingService,
  ...
  @ApiParam({ name: 'userId', required: true, description: 'Id пользователя', example: '3b809c42-2824-46c1-9686-dd666403402a' })
  async create(@Param('userId') userId: string, @Body() roleSettingCreateDto: RoleSettingCreateDto, @Ip() ip: string): Promise<RoleSetting>{
      const role = await this.roleService.findOneById(roleSettingCreateDto.roleId)
      roleSettingCreateDto.role = role;
      const createdRule = await this.roleSettingService.create(roleSettingCreateDto);
      this.logger.info(`${yellow('OK!')} - ${red(ip)} - roleSettingCreateDto: ${JSON.stringify(roleSettingCreateDto)} - Создана настройка!`);
      return createdRule;
  }
