import { Body, Controller, HttpStatus, Inject, Patch, UseGuards } from "@nestjs/common";
    ...
    async updateOrderNumbers(
        ...
        const updatePanelToStatisticPromises = panelToStatisticUpdateDtos.map(
            async (panetlToStatisticUpdateDto) => {
                const updatedPanelToStatisticId = await this.panelToStatisticService.update(panetlToStatisticUpdateDto._id, panetlToStatisticUpdateDto);
